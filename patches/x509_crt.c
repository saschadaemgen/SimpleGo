                                   ...
                                   void *p_ctx)
{
    ...
    if ((ret = mbedtls_x509_get_sig_alg(&crt->sig_oid, &sig_params1,
                                        ...
                                        &crt->sig_opts)) != 0) {
        /* SIMPLEX PATCH: Ignore unknown sig alg (Ed448/Ed25519) */
        crt->sig_md = MBEDTLS_MD_NONE;
        crt->sig_pk = MBEDTLS_PK_NONE;
        crt->sig_opts = NULL;
    ...
    if ((ret = mbedtls_pk_parse_subpubkey(&p, end, &crt->pk)) != 0) {
        /* SIMPLEX PATCH: Skip unknown PK (Ed448/Ed25519) */
        size_t pk_len;
        unsigned char *p_tmp = crt->pk_raw.p;
        mbedtls_pk_init(&crt->pk);
        /* Parse SEQUENCE header to get length and skip it */
        if (mbedtls_asn1_get_tag(&p_tmp, end, &pk_len, MBEDTLS_ASN1_CONSTRUCTED | MBEDTLS_ASN1_SEQUENCE) == 0) {
            p = p_tmp + pk_len;
        }
    }
