...
- **🏗️ Modular Architecture** — Monolithic ~1800 line main.c split into 8 dedicated modules
- **🔌 Peer Connection** — New smp_peer.c module for connecting to peer's SMP server
- **✅ Server Accepts Confirmation** — AgentConfirmation sent and acknowledged with "OK"
- **🐛 DH Key Fix** — Invitation URIs use Standard Base64, not Base64URL
...
// Convert +/ to -_  (Standard Base64 → Base64URL)
for (int x = 0; x < len; x++) {
...
| Peer Server Connection | ✅ Working |
| TLS + SMP Handshake with Peer | ✅ Working |
| AgentConfirmation Sent | ✅ Server: OK |
| App Shows "Connected" | 🔧 Format Issue |

---

## 🔍 Known Issue: App Not Showing "Connected"
...
- Previous: v0.1.13-alpha — Message Type Fix + Peer Queue
- Next: v0.1.15-alpha — encConnInfo Format Fix (planned)
