...
The result is a new class of device: a dedicated, smartphone-independent messenger with hardware-enforced security that no existing product on the market provides. Not a modified phone. Not an app. A purpose-built machine whose only job is to send and receive encrypted messages.
...
The encrypted communications market is projected to reach **$8.36 billion by 2034**. Yet after exhaustive analysis of more than 70 devices across consumer, military, and open-source domains, no existing product combines even four of these six security properties:
...
The maximum overlap found in any single device is **three out of six** — achieved only by LoRa mesh devices (Meshtastic, Reticulum) that satisfy bare-metal firmware, no baseband, and open source, but entirely lack multi-layer encryption, identity-free design, and hardware secure elements.

SimpleGo occupies the verified whitespace between two distant clusters in the design space: **high-assurance military devices** with classified algorithms and tamper resistance but proprietary, closed-source platforms — and **open-source mesh devices** with transparency and no baseband but zero hardware security features. No product, prototype, or published concept bridges this gap.

---

## The Smartphone Problem

Every hardened smartphone on the market retains a cellular baseband processor — a secondary computer running proprietary firmware with direct memory access. Academic research (BASECOMP, USENIX Security '23) has found critical exploitable vulnerabilities in these chips, including NAS AKA bypasses and hundreds of undisclosed commands. Even the most security-conscious phones merely attempt to isolate the baseband rather than eliminate it.
...
SimpleGo eliminates entire categories of attacks. No browser means no browser exploits. No app installation means no malware vector. No baseband with DMA means no cellular-based memory attacks. The trusted computing base shrinks by three orders of magnitude.

---

## Triple-Layer Encryption

SimpleGo implements the full SimpleX encryption architecture — three cryptographically independent layers per message, each defending against a different threat:
...
Content padding to a fixed 16KB block size is applied at each layer. No other messaging protocol implements comparable per-message triple encryption. Signal uses two layers. Matrix uses two layers. Among hardware devices, the maximum found was two. SimpleGo is the first hardware implementation of this architecture.

---

## Hardware Tiers

SimpleGo defines three hardware security tiers for different threat models. The design philosophy starts with the highest-specification Tier 3 and creates lower tiers by removing components — ensuring architectural consistency across the entire product line.
...
The dual-vendor secure element architecture means a vulnerability in one manufacturer's chip (such as the Eucleak side-channel attack on Infineon SLE78, disclosed 2024) does not compromise the device. Cryptographic operations are split across vendors so that both must be compromised simultaneously.
...
The core protocol stack is functional with bidirectional encrypted messaging confirmed between ESP32 hardware and the official SimpleX Chat application.
...
| Persistent crypto state | ✅ Complete | Survives reboots |
...
| Contact management UI | 🔧 In Progress | |
| SD card encrypted history | 📋 Planned | AES-256-GCM per-contact |
...
**Tier 1 — DIY Device** — A fully assembled and tested T-Deck Plus, pre-flashed with SimpleGo firmware, ready to use. Fully open source — build your own or flash updates anytime.
...
- Design and manufacture custom PCBs for Tier 2 and Tier 3 devices
- Establish the supply chain for secure element components from three independent vendors
- Commission an independent security audit of the protocol implementation
- Build initial production inventory
- Complete German UG company formation and regulatory compliance

The Kickstarter model aligns with SimpleGo's values: direct community funding, full transparency about development progress and costs, and backers who care about privacy as a fundamental right — not a premium feature.
...
SimpleGo does not claim to protect against:
