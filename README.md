...
[![Version](https://img.shields.io/badge/version-0.1.18--alpha-orange.svg)](#project-status)
...
All messages are padded to a fixed 16 KB block size at each layer, eliminating message-length side channels. No other messaging protocol (Signal, Matrix, or any hardware device surveyed) implements comparable per-message triple encryption.
...
**Cryptographic erasure on contact delete.** When a contact is removed, all associated NVS keys are overwritten with cryptographic-strength random data before deletion, preventing forensic recovery.

## Hardware Architecture

SimpleGo is built around a Hardware Abstraction Layer (HAL). The complete protocol stack -- SMP, X3DH, Double Ratchet, all three encryption layers, the agent protocol, and the UI framework -- is fully device-independent. A new hardware platform is supported by implementing five HAL interface files. Everything above the HAL comes for free.

The LilyGo T-Deck Plus is the first supported device and the current development platform. It is not the only device SimpleGo runs on -- it is the first. Additional HAL implementations for other microcontroller boards and form factors are in progress.

SimpleGo defines three security tiers. The tiers describe hardware security capability levels, not specific devices. Multiple devices can implement each tier.

### Tier 1

Tier 1 targets accessible hardware: microcontroller boards available off-the-shelf, flashable by anyone, with a single hardware secure element for key storage. The full protocol stack including triple-layer encryption, multi-contact architecture, encrypted SD chat history, and WiFi management runs completely on Tier 1 hardware.

**Current Tier 1 reference device: LilyGo T-Deck Plus**

| Component | Specification |
|-----------|--------------|
| MCU | ESP32-S3, dual-core LX7, 240 MHz, 8 MB PSRAM |
| Secure element | ATECC608B (Microchip) |
| Storage | MicroSD, AES-256-GCM encrypted chat history |
| Connectivity | WiFi 802.11 b/g/n |
| Input | Physical QWERTY keyboard, trackball, touch |
| Security features | Secure Boot v2, Flash Encryption, eFuse-bound NVS encryption |

### Tier 2

Tier 2 uses a custom PCB designed for the SimpleGo security model. It introduces dual-vendor secure elements, active tamper detection, and physical separation between processing and communication subsystems.

| Component | Specification |
|-----------|--------------|
| MCU | STM32U585 (ARM Cortex-M33 with TrustZone, 160 MHz) |
...
| Enclosure | CNC-milled aluminium |
...
Tier 3 represents the maximum achievable security in a portable device. Triple-vendor secure elements from three independent manufacturers, a dedicated tamper supervisor with hardware key zeroization, full environmental monitoring, and a potted enclosure designed to defeat physical probing.

| Component | Specification |
|-----------|--------------|
| MCU | STM32U5A9 (ARM Cortex-M33, 4 MB Flash, TrustZone) |
...
| Tamper response | Temperature, voltage, light, vibration, mesh continuity monitoring |
| Connectivity | WiFi 6, 4G LTE / 5G NR (isolated, no DMA), LoRa, satellite (optional) |
| Enclosure | Potted CNC aluminium with aluminium-filled epoxy |
...
Cellular modules on Tier 2 and Tier 3 function purely as data modems through a defined serial interface with no DMA access and no shared memory with the main processor. The baseband attack surface is present but fully isolated -- unlike any smartphone-based approach.

## Project Status

The core protocol stack is functional and verified against the official SimpleX Chat application across multiple simultaneous contacts, including delivery receipts, encrypted chat history, and persistent crypto state across reboots.
...
| Bidirectional messaging | Complete | Verified across multiple simultaneous contacts |
...
| Persistent crypto state | Complete | Survives reboots, NVS-backed |
...
| Sliding window chat view | Complete | Constant memory, scroll loads from SD |
...
| eFuse-bound NVS encryption | Production release | Device-unique AES-256 key burned at provisioning |
| Post-quantum crypto (Kyber) | Production release | Ships together with eFuse release |
| Private Message Routing | Planned | Prevents server-side IP graph construction |
| Keep-alive (PING/PONG) | Planned | |

### Memory Footprint (T-Deck Plus)
...
The Protocol and Application layers are identical across all devices and all security tiers. Only the HAL implementations change. Adding a new hardware platform means implementing five interface files -- the entire protocol stack, encryption architecture, and UI framework come for free.
...
Full build documentation and HAL porting guide: [wiki.simplego.dev](https://wiki.simplego.dev)
...
| SimpleGo Tier 2 (custom PCB) | STM32U585 | PCB design phase |
| SimpleGo Tier 3 Vault (custom PCB) | STM32U5A9 | PCB planning |

Adding support for new hardware requires implementing the HAL interfaces defined in `devices/template/`. The protocol layer requires no modification.

## Documentation

Full documentation is available at [wiki.simplego.dev](https://wiki.simplego.dev), including:

- [SMP in C Guide](https://wiki.simplego.dev/smp-in-c) -- the only documentation in existence for implementing the SimpleX Messaging Protocol in C
- [Architecture Overview](https://wiki.simplego.dev/architecture)
- [Security Model and Threat Model](https://wiki.simplego.dev/security)
- [T-Deck Plus Hardware Reference](https://wiki.simplego.dev/hardware/t-deck-plus)
- [HAL Porting Guide](https://wiki.simplego.dev/contributing)
- [Protocol Analysis](docs/protocol-analysis/) -- 40 parts, 33,000+ lines of implementation documentation
...
**What SimpleGo does not protect against:** compromise of SMP relay servers (mitigated by the protocol's metadata protection architecture, not eliminated), physical attacks against Tier 1 devices without secure enclosure (addressed in Tier 2 and 3), and network-level traffic analysis (Private Message Routing is planned for a future release).
...
- [SimpleX Chat](https://simplex.chat/) for the SimpleX Messaging Protocol and continued technical guidance
