...
int simplex_secretbox_open_debug(uint8_t *plain, 
                                  ...
                                  const char *tag)
{
    printf("\n[%s] SimpleX E2E Decrypt Debug\n", tag);
    printf("=========================================\n");
    
    if (cipherlen < 16) {
        printf("ERROR: cipherlen %zu < 16\n", cipherlen);
        return -1;
    }
    
    const uint8_t *mac = cipher;
    const uint8_t *ct = cipher + 16;
    size_t ct_len = cipherlen - 16;
    ...
    printf("Nonce:         ");
    for (int i = 0; i < 24; i++) printf("%02x", nonce24[i]);
    printf("\n");
    ...
    printf("\nStep 1: HSalsa20(dh, zeros[16])\n");
    printf("  Input:       zeros[16]\n");
    printf("  Subkey1:     ");
    for (int i = 0; i < 32; i++) printf("%02x", subkey1[i]);
    printf("\n");
    ...
    printf("\nStep 2: HSalsa20(subkey1, nonce[8:24])\n");
    printf("  Input:       ");
    for (int i = 8; i < 24; i++) printf("%02x", nonce24[i]);
    printf("\n");
    printf("  Subkey2:     ");
    for (int i = 0; i < 32; i++) printf("%02x", subkey2[i]);
    printf("\n");
    ...
    printf("\nStep 3: Salsa20(subkey2, nonce[0:8])\n");
    printf("  Nonce:       ");
    for (int i = 0; i < 8; i++) printf("%02x", nonce24[i]);
    printf("\n");
    printf("  Poly key:    ");
    for (int i = 0; i < 32; i++) printf("%02x", stream[i]);
    printf("\n");
    
    // Step 4: Verify MAC
    uint8_t computed_mac[16];
    crypto_onetimeauth(computed_mac, ct, ct_len, stream);
    
    printf("\nStep 4: Poly1305 MAC verify\n");
    printf("  Expected:    ");
    for (int i = 0; i < 16; i++) printf("%02x", mac[i]);
    printf("\n");
    printf("  Computed:    ");
    for (int i = 0; i < 16; i++) printf("%02x", computed_mac[i]);
    printf("\n");
    ...
    printf("\nStep 5: Decrypted plaintext (%zu bytes)\n", ct_len);
    printf("  First 32:    ");
    for (size_t i = 0; i < 32 && i < ct_len; i++) printf("%02x", plain[i]);
    printf("\n");
    
    // Try to print as ASCII if printable
    printf("  As ASCII:    ");
    for (size_t i = 0; i < 64 && i < ct_len; i++) {
